...
import bean.{CourseShoppingCart, SaleCourse}
...
object SkewJoinTuning {
  ...
  def scatterBigAndExpansionSmall( sparkSession: SparkSession ): Unit = {
    ...
    //TODO 2、将倾斜的key打散  打散36份 【一个并行度处理一个】
    val newCourseShoppingCart = skewCourseShoppingCart.mapPartitions(
      // 每个key对应的一个分区的数据操作
      ( partitions: Iterator[Row] ) => {
        partitions.map(item => {
